...
/* A spray posts to BOTH tables under one source_id. When a row is linked,
   edit the whole record in ONE form and re-record both sides together
   (edit_spray). Rows with no source_id (legacy/import) edit the single table. */
function openEntry(row, kind){
  if(row.source_id) return sprayCombined(row.source_id);
  return kind==="farmer" ? farmerForm(row) : acreForm(row);
}
async function sprayCombined(source){
  const m=$("main");
  m.innerHTML='<div class="card muted" style="margin-top:12px">Loading entry…</div>';
  const [{data:aR},{data:fR},{data:crops}]=await Promise.all([
    sb().from("acre_entries").select("*, loc:location_id(name)").eq("source_id",source).limit(1),
    sb().from("farmer_sprays").select("*").eq("source_id",source).limit(1),
    sb().from("crops").select("id,name").eq("active",true).order("name")
  ]);
  ...
  m.innerHTML=`<button class="btn sm" id="eqBack">← Back to Entries</button>
    ...
    <div class="fgrid">
      ...
      <div class="field"><label>Pilot</label><input id="s_pilot" value="${val(a.pilot_name||f.pilot_name)}" ${billed?'disabled':''}></div>
      <div class="field"><label>Acres</label><input id="s_acres" type="number" step="any" value="${val(a.acres!=null?a.acres:f.acre)}" ${billed?'disabled':''}></div>
      <div class="field"><label>Client rate (₹/acre)</label><input id="s_crate" type="number" step="any" value="${val(a.client_rate)}" ${billed?'disabled':''}></div>
      <div class="field"><label>Farmer rate (₹/acre)</label><input id="s_frate" type="number" step="any" value="${val(a.farmer_rate)}" ${billed?'disabled':''}></div>
      <div class="field"><label>Crop</label><select id="s_crop" ${billed?'disabled':''}><option value="">— crop —</option>${cl.map(c=>`<option value="${c.id}" ${String(a.crop_id||'')===String(c.id)?'selected':''}>${esc(c.name)}</option>`).join("")}</select></div>
      <div class="field"><label>Medicine / Chemical</label><input id="s_chem" value="${val(a.chemical||f.chemical_company)}" ${billed?'disabled':''}></div>
      <div class="field"><label>Farmer</label><input id="s_farmer" value="${val(f.farmer_name)}" ${billed?'disabled':''}></div>
  ...
  if($("s_del")) $("s_del").addEventListener("click",async()=>{
    ...
    const { error }=await sb().rpc("delete_spray",{ p_source:source });
    if(error){ alert(error.message); return; }
    window.OPS.audit("deleted","spray",source,""); window.OPS.flashTop("Deleted ✓"); view();
  });
}
...
function acreForm(r){
  const m=$("main");
  if(r.farmer_doc_id||r.client_doc_id||r.vendor_doc_id||r.farmer_billed_override||r.client_billed_override){
    m.innerHTML='<button class="btn sm" id="eqBack">← Back to Entries</button><div class="card" style="margin-top:12px"><div class="callout warn">🔒 <b>This entry is on an invoice</b> and is locked. Credit or withdraw that invoice (Finance → Acre Invoicing) to release it for editing.</div></div>';
    $("eqBack").addEventListener("click",view); return;
  }
  const admin = window.OPS.isAdmin() || (window.OPS.isApprover && window.OPS.isApprover());
