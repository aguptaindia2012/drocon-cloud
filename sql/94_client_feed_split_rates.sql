-- ============================================================================
-- 94. Client feed: split the billing into farmer-rate and client-rate
-- ----------------------------------------------------------------------------
-- Clients want to see the FARMER rate and the CLIENT rate separately, not one
-- combined figure. acre_entries carries farmer_rate/client_rate (sql/18), with
-- the effective value falling back to the location's rates (sql/35):
--     farmer_rate = coalesce(entry.farmer_rate, location.farmer_rate, 0)
--     client_rate = coalesce(entry.client_rate, location.client_rate, 0)
-- Amounts are acres * the matching rate; total amount = farmer + client.
-- Recreates the feed functions (return type changes). Same scoping as sql/92.
-- ============================================================================
...
returns table(
  ...
  farmer_amount numeric, client_amount numeric, amount numeric, open_issue boolean)
